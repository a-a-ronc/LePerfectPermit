...
export async function generateCoverLetterWithAI(
  ...
  try {
    ...
    documents.forEach(doc => {
      ...
      // Remove copy quantities and clean filename (e.g., "(8 copies)", "(22 copies)")
      const cleanFileName = doc.fileName
        .replace(/\s*\(\d+\s*copies?\)\s*/gi, '')
        .replace(/&nbsp;/g, ' ')
        .trim();
    ...
    // Create dynamic numbering for categories that have files
    const categoriesWithFiles = desiredOrder.filter(cat => {
