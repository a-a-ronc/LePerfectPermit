...
export default function ProjectPage() {
  ...
  const onSubmit = async (data: ProjectFormValues) => {
    try {
      ...
      const result = await response.json(); //e.g {projectId: 42}
      const newProjectId = result.projectId ?? result.id;
      
      // invalidate queries and close dialog on success
      queryClient.invalidateQueries({ queryKey: ["/api/projects"] });
  ...
  return (
    <div className="min-h-screen bg-muted dark:bg-gray-900">
      {/* Top Navigation Bar */}
      <div className="bg-card dark:bg-gray-800 p-4 shadow-sm flex items-center justify-between">
          <nav className="flex" aria-label="Breadcrumb">
            <ol className="flex items-center space-x-2">
              <li>
                <Link href="/">
                  <div className="text-muted-foreground dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300 cursor-pointer">Dashboard</div>
                </Link>
              </li>
              <li className="flex items-center">
                ...
                <span className="ml-2 text-muted-foreground dark:text-gray-400">Projects</span>
              </li>
            </ol>
          </nav>
      </div>
      ...
      <div className="p-6 overflow-auto h-[calc(100vh-64px)]">
          <div className="mb-6 flex flex-wrap justify-between items-center">
            <div>
              ...
              <p className="text-muted-foreground">Manage all your high-piled storage permit projects.</p>
            </div>
