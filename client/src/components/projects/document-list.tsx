    ...
    <div className="overflow-hidden">
      {categoriesWithDocuments.map(({ category, document }) => {
        ...
        if (!document) {
          ...
          return (
            <div key={rowKey} id={`category-${category}`} className="border-b border-border bg-card transition-colors duration-300">
              <div className="flex flex-wrap justify-between p-4">
                <div className="w-full md:w-auto flex items-center">
                  <FileText className="h-6 w-6 text-gray-400 mr-3" />
                  <div>
                    <h3 className="font-medium">{formatDocumentCategory(category)}</h3>
                    <p className="text-sm text-muted-foreground">{getDocumentCategoryDescription(category)}</p>
        ...
        return (
          <div key={rowKey} id={`category-${doc.category}`} className={`border-b border-border ${isExpanded ? 'bg-muted' : 'bg-card'}`}>
            <div 
            ...
            >
              <div className="w-full md:w-auto flex items-center">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">{getDocumentCategoryDescription(doc.category)}</p>
                </div>
              </div>
              <div className="mt-3 md:mt-0 flex items-center gap-2">
                ...
                <Button 
                  variant="ghost" 
                  size="sm"
                  className="flex items-center h-7 w-7 p-0 text-red-500 hover:text-red-700 hover:bg-red-50 dark:bg-red-900"
              ...
              <div className="px-4 pb-4">
                <div className="p-3 bg-card rounded-lg border border-border">
                  ...
                  {doc.status === DocumentStatus.REJECTED && doc.comments && (
                    <div className="mb-3 p-3 rounded border bg-red-50 dark:bg-red-900 border-red-200 dark:border-red-700">
                  ...
                  {doc.status === DocumentStatus.APPROVED && doc.comments && (
                    <div className="mb-3 p-3 rounded border bg-green-50 dark:bg-green-900 border-green-200 dark:border-green-700">
                              ...
                              {displayItems.map((item, idx) => (
                                <div key={idx} className="flex items-start gap-2">
                                  {item.checked ? (
                                    ...
                                    <div className="h-4 w-4 mt-0.5 rounded border border-amber-300 bg-card"></div>
                                  )}
                      ...
                      <div className="mt-3 md:mt-0">
                        <div className="flex items-center text-sm text-muted-foreground">
                          {doc.status === DocumentStatus.APPROVED ? (
                            ...
                            <>
                              <span className="h-6 w-6 rounded-full bg-yellow-100 dark:bg-yellow-900 flex items-center justify-center mr-2">
      ...
      <Dialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <DialogContent className="sm:max-w-md">
          ...
          <div className="py-4">
            {documentToDelete && (
              <div className="flex items-center p-3 bg-muted rounded-md border border-border">
                <FileText className="h-5 w-5 text-muted-foreground mr-2" />
                <div>
                  <p className="font-medium text-sm">{documentToDelete.fileName}</p>
                  <p className="text-xs text-muted-foreground">{formatDocumentCategory(documentToDelete.category)}</p>
                </div>
              </div>
            )}
          </div>
