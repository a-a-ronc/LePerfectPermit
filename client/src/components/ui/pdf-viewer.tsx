...
export function PDFViewer({ document }: PDFViewerProps) {
  ...
  if (document.fileType === 'text/plain' && fileContent) {
    return (
      <div className="w-full h-full overflow-auto p-6 text-gray-800 bg-card shadow-inner relative">
        <div className="absolute top-2 right-2 flex gap-2">
          ...
          <button
            ...
            className="px-3 py-1.5 bg-gray-200 dark:bg-gray-700 backdrop-blur text-gray-800 rounded text-xs font-medium hover:bg-gray-300"
          >
            Open in New Window
          </button>
        </div>
  ...
  if (error || !fileUrl) {
    return (
      <div className="w-full h-full flex items-center justify-center">
        <div className="p-4 border border-red-200 dark:border-red-700 bg-red-50 dark:bg-red-900 rounded-md text-red-700 text-sm max-w-md">
